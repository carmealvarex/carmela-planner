...
export function Recordatorios({ events, recordatoriosSueltos, setRecordatoriosSueltos, onOpenEvent }) {
  ...
  const todos = [...deEventos, ...sueltos].sort((a, b) => a.dias - b.dias);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="p-5 rounded" style={{ background: CARD, border: `1px solid ${LINE}` }}>
        <h3 style={{ fontFamily: FONT_HEAD, fontSize: 20, color: INK, marginBottom: 12 }}>
          Todos los recordatorios {todos.length > 0 ? `(${todos.length})` : ""}
        </h3>
        {todos.length === 0 ? (
          <p style={{ fontFamily: FONT_BODY, fontSize: 13, color: MUTED }}>No hay recordatorios cargados todavía.</p>
        ) : (
